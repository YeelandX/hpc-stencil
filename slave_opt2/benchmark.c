...
int parse_args(dist_grid_info_t *info, int *nsteps, int *stencil_type, int argc,
               char **argv);
...
int read_file(ptr_t grid, const char *file_name, const file_helper_t *h,
...
extern check_result_t check_answer(cptr_t ans0, cptr_t ans1,
                                   const dist_grid_info_t *info,
                                   MPI_Comm active_procs);

#define CHECK(err, err_code) \
  if (err)                   \
  {                          \
    return err_code;         \
  }
...
int main(int argc, char **argv)
{
  ...
  if (status == 0)
  {
    ...
    {
      double t = end - start;
      double gflops = 1e-9 * info.global_size_x * info.global_size_y *
                      info.global_size_z * nt * (type * 2 - 1) / t;

      printf("%d-point stencil - %s:\nSize (%d x %d x %d), Timestep %d\n", type,
             version_name, info.global_size_x, info.global_size_y,
             info.global_size_z, nt);
...
void print_help(const char *argv0, int p_id)
{
  ...
  {
    printf(
        "USAGE: %s <stencil-type> <nx> <ny> <nz> <nt> <input-file> "
...
{
  int status;
  if (info->local_size_x > 0 && info->local_size_y > 0 &&
      info->local_size_z > 0)
  {
    status = 0;
  }
...
void create_file_helper(file_helper_t *h, const dist_grid_info_t *info)
{
  ...
  subsize[0] = info->local_size_x;
  subsize[1] = info->local_size_y;
  subsize[2] = info->local_size_z;

  size[0] = info->global_size_x;
  size[1] = info->global_size_y;
  size[2] = info->global_size_z;
  start[0] = info->offset_x;
  start[1] = info->offset_y;
  start[2] = info->offset_z;
  MPI_Type_create_subarray(3, size, subsize, start, MPI_ORDER_FORTRAN,
                           DATA_TYPE, &h->file_view);
  ...
  start[0] = info->halo_size_x;
  start[1] = info->halo_size_y;
  start[2] = info->halo_size_z;
  MPI_Type_create_subarray(3, size, subsize, start, MPI_ORDER_FORTRAN,
                           DATA_TYPE, &h->array_center);
              ...
              MPI_Comm active_procs)
{
  ...
  MPI_Status status;
  int count;
  int ret = MPI_File_open(active_procs, file_name, MPI_MODE_RDONLY,
                          MPI_INFO_NULL, &input);
  CHECK_ERROR(ret, ret)
  ret = MPI_File_set_view(input, 0, DATA_TYPE, h->file_view, "native",
                          MPI_INFO_NULL);
  CHECK_ERROR(ret, ret)
...
{
  ptr_t tmp;
  int x_start = info->halo_size_x,
      x_end = info->local_size_x + info->halo_size_x;
  int y_start = info->halo_size_y,
      y_end = info->local_size_y + info->halo_size_y;
  int z_start = info->halo_size_z,
      z_end = info->local_size_z + info->halo_size_z;
