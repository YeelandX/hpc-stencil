...
check_result_t check_answer(cptr_t ans0, cptr_t ans1,
                            const dist_grid_info_t *info, MPI_Comm active) {
  ...
  double norm_1_tmp_local = 0.0, norm_2_tmp_local = 0.0,
         norm_inf_tmp_local = 0.0;
...
#pragma omp simd
      ...
      for (int x = x_start; x < x_end; ++x) {
        double err = fabs(ans0[INDEX(x, y, z, ldx, ldy)] -
                          ans1[INDEX(x, y, z, ldx, ldy)]);
        norm_1_tmp_y += err;
  ...
  result.norm_1 = norm_1 / (info->global_size_x * info->global_size_y *
                            info->global_size_z);
  result.norm_2 = sqrt(norm_2 / (info->global_size_x * info->global_size_y *
                                 info->global_size_z));
