 ...
 * (local_size_x + halo_size_x * 2) * (local_size_y + halo_size_y * 2) *
 * (local_size_y + halo_size_x * 2)
 *
 * the element `l[halo_size_x + x][halo_size_y + y][halo_size_z + z]` in local
 * array represents the element `g[offset_x + x][offset_y + y][offset_z + z]` in
 * global array
 */
typedef struct
{
...
// blocking
#define INDEX2(xx, yy, ldxx) ((xx) + (ldxx) * (yy))
